...
if info.isControl == 1
    ...
    binaryVec = [zeros(1,5), ones(1,5)];
    randInds  = randperm(10);
...
while preTestPref == 0
    for testNum = 1:4
        ...
        [data_out]=VR_laserPosition_test_tcp(tempEnvStruct.both_go_noCool, info.trialTime,info.exName,[trialName],info.directory,...
            info.XYstart,info.rotGain,info.sampleRate,info,ao,info.preOnTime,info.evidenceTime,info.checkEnv, tcpipServer, flipThisTrial);
  ...
  for rep = 1:20
    ...
    %% run experiment trial
        [data_out]  =   VR_laserPosition_train_tcp(envToUse, info.trialTime,info.exName,[trialName],info.directory,...
            info.XYstart,info.rotGain,info.sampleRate,info, ao,info.preOnTime,info.evidenceTime, info.checkEnv, tcpipServer);
        data = data_out;
