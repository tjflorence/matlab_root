function [data_out]= VR_laserPosition_test_tcp(input_tempEnv,experimentLength, exName, fileName, directory,...
...
[visInit, patternStruct, binvals] = makePatternStruct;
...
timer1      =   tic;
while (data.timeStamp(data.count) < data.experimentLength) && (data.OOB_frames < (info.sampleRate*60)) && (data.deadEnd_frames<(info.sampleRate*60)) && (data.edgeFrames < (info.sampleRate*10))  && (data.safeFrames < (10) )
...
Vfwd=sum(x0); %integration rotation for the entire 1/info.sampleRateth of a second
Vss=sum(y0);
Omega=sum(x1);
...
elseif data.trial_state_current == 4 %% now, fly has full closed-loop control
    ...
    if sqrt((data.Xpos(data.count)-data.arenaRadius)^2 + (data.Ypos(data.count)-data.arenaRadius)^2) >= data.arenaRadius*.9
...
%% write this data to tcp 
updateDisplay(normalizedX, normalizedY, data.flyTheta(data.count), patternCode) 
