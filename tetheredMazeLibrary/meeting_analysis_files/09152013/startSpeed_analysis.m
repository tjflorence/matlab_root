...
for aa = 1:11
    ...
    for bb = 1:14
        ...
        oflies(bb).speed_5(aa,:) = data.speed_CL_sm(1:100)./data.ticks_per_mm;
    end
...
for aa = 1:28
    oflies(aa).inst_std = nanstd(oflies(aa).speed_5);
end
...
for aa = 1:7
    ...
    for bb = 1:14
        load(dataFiles(bb).name)
        try
        nflies(bb).speed_5(aa,:) = data.speed_CL_sm(1:100)./data.ticks_per_mm;
        catch
        nflies(bb).speed_5(aa,1:length(data.speed_CL_sm)) = data.speed_CL_sm(1:end)./data.ticks_per_mm;
        end
    end
...
for aa = 1:28
    nflies(aa).inst_std = nanstd(nflies(aa).speed_5);
end
...
for aa = 1:28
    ...
    s1 = subplot(4,7,aa)
    h1 = ciplot(nanmean(oflies(aa).speed_5)-oflies(aa).inst_std, nanmean(oflies(aa).speed_5)+oflies(aa).inst_std, 1:length(nanmean(oflies(aa).speed_5)+oflies(aa).inst_std), [86 60 255]./255)
    set(h1, 'FaceAlpha', .5)
    hold on
    plot(nanmean(oflies(aa).speed_5), 'Color',[86 60 255]./255,  'LineWidth', 1.5)
 
    
    h1 = ciplot(nanmean(nflies(aa).speed_5)-nflies(aa).inst_std, nanmean(nflies(aa).speed_5)+nflies(aa).inst_std, 1:length(nanmean(nflies(aa).speed_5)+nflies(aa).inst_std), [255 171 60]./255)
    set(h1, 'FaceAlpha', .5)
    hold on
    plot(nanmean(nflies(aa).speed_5), 'Color', [255 171 60]./255, 'LineWidth', 1.5)   
    ...
    if aa == 1
     ...
     text(-10, 50, 'stripe fix trigger, n = 7', 'FontSize', 20, 'Color', [255 171 60]./255, 'FontWeight', 'Bold' )
     text(-10, 45, 'no stripe fix trigger, n = 11', 'FontSize', 20, 'Color',[86 60 255]./255,'FontWeight', 'Bold' )
...
export_fig('early_speed_analysis', '-png')
