...
timer1      =   tic;
while (data.timeStamp(data.count) < data.experimentLength) && (data.OOB_frames < (info.sampleRate*120)) && (data.deadEnd_frames<(info.sampleRate*60)) && (data.edgeFrames < (info.sampleRate*10))  && (data.safeFrames < (10) )
...
Vfwd=sum(x0)*.7071; %integration rotation for the entire 1/info.sampleRateth of a second
Vss=sum(y0)*.7071;
Omega=sum(x1);
