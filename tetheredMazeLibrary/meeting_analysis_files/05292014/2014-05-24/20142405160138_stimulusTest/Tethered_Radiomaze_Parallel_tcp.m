...
while preTestPref == 0
    for testNum = 1:4
        ...
        if flipThisTrial == 0
            refEnv      = tempEnvStruct.ref_env;
            startVec    = info.XYstart(1,:);
            testEnv     = tempEnvStruct.test_env;
        elseif flipThisTrial == 1
            refEnv      = tempEnvStruct.ref_en;
        ...
        [data_out]=VR_laserPosition_test_tcp(testEnv, info.trialTime,info.exName,[trialName],info.directory,...
                                            startVec,info.rotGain,info.sampleRate,info,ao,info.preOnTime,...
                                            info.evidenceTime,refEnv, tcpipServer, flipThisTrial);
  ...
  for rep = 1:numReps
    %while nextStage == 0
        ...
        if info.flipCoolSpot(envRep) == 1
            envToUse    = tempEnvStruct.sw_start_flipped;
        else
            envToUse    = tempEnvStruct.sw_start;
        end
