...
text(-2, 9.5, 'exp flies: cool spot on right', 'Rotation', 90)
...
for aa = 1:7
    ...
    cd(cfly(aa).tr1)
    load('summaryVec.mat')
    cfly(aa).tr1_data = summaryVec;
    
    
    cd(cfly(aa).tr2)
    load('summaryVec.mat')
    cfly(aa).tr2_data = summaryVec;    
...
for aa = 1:7
    control_colorSummaryMatrix(aa,:) = [cfly(aa).tr1_data cfly(aa).tr2_data 6*ones(1,2)];
end
...
text(-2, 7.5, 'control flies: random cool spot', 'rotation', 90)
...
summaryPlotMatrix = [.4 .75 .8 .75;...
 ...
 control_summaryPlotMatrix = [.5  .25 .4 .75;...
                              .5  .75 .7 .5;...
                              .6  .5  .4 .25;...
                              .3  0   .5 .5;...
                              .5  .25 1  .25;...
...
set(s2, 'position', [sizePlot(1) sizePlot(2) sizePlot(3)*1.105 sizePlot(4)*1.3])

sizePlot = get(s3, 'Position')
set(s3, 'position', [sizePlot(1)-.35 sizePlot(2)+.2 sizePlot(3)*1.5 sizePlot(4)*.8])
text(4.5,.9, 'exp condition: go right', 'FontWeight', 'bold', 'Color', 'r')
text(4.5,.8, 'control condition: random', 'FontWeight', 'bold', 'Color', 'k')
